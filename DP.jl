...
begin
    ...
    function createSystemNoExt( H_ARG::DataType)
        @independent_variables t
        # Define states and their derivatives explicitly
        @variables x(t) θ1(t) θ2(t) dx(t) dθ1(t) dθ2(t)
        @parameters  mc m1 m2 L1 L2 g H(::Real, ::Real, ::Real, ::Real, ::Real, ::Real, ::H_ARG) ARGS::H_ARG
        ...
        V = g*(m1*l1 + m2*L1)*cos(θ1) + m2*g*l2*cos(θ2)
        L = T - V
        EL = expand_derivatives(d(Differential(dx)(L))) - expand_derivatives(Differential(x)(L)) ~ 0;  # force(θ1, x, t)
        push!(eqs, EL );
        EL = expand_derivatives(d(Differential(dθ1)(L))) - expand_derivatives(Differential(θ1)(L)) ~ 0;#H(θ1, θ2, t)  # force(θ1, x, t)
        push!(eqs, EL );
        EL = expand_derivatives(d(Differential(dθ2)(L))) - expand_derivatives(Differential(θ2)(L)) ~ 0;#H(θ1, θ2, t)  # force(θ1, x, t)
    ...
    function createProblem(sys, H, arginit, IC=nothing, tspan = (0,20))
        # If no initial conditions provided, use defaults
        if IC === nothing
            IC = [sys.x=>0.0, sys.θ1=>0.0, sys.θ2=>0.0, sys.dx=>0.0, sys.dθ1=>0.0, sys.dθ2=>0.0]
        end
        
        # Check if IC is array-like or dictionary-like
        if IC isa AbstractArray
            # Convert array to dictionary of pairs
            IC = [
                sys.x => IC[1],
                sys.θ1 => IC[2],
                sys.θ2 => IC[3],
